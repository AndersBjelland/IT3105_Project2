...
class BasicClientActor(BasicClientActorAbs):
    ...
    def handle_get_action(self, state):
        ...
        # In the OHT, player 1 and player 2 will be represented similarly to how we do it.
        # Player 1 attempts to build on the SW - NE diagonal, while player 2 attempts to build on the NW - SE diagonal.
        # However, we use a different designation of 'player 1' and 'player 2' compared to the OHT. We use player 1 to
        # Strictly mean the starting player.
        # As such, if player 2 does the first move in the OHT, it is 'really' player 1, seen from the perspective of our implementation
        # Thus, we need to flip the board if we are player 2
        # P1 attempts to span all rows, while P2 attempts to span all columns
        flip = self.starting_player == 2

        grid = np.reshape(board, (self.size, self.size)).T
        if flip:
            grid = grid.T
            ones = grid == 1
            twos = grid == 2
    ...
    def handle_series_start(self, unique_id, series_id, player_map, num_games, game_params):
        ...
        print(f'series start with params:')
        print({
            'unique_id': unique_id,
            'series_id': series_id,
