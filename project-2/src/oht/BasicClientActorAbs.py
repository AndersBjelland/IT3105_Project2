...
class BasicClientActorAbs(ABC):
    ...
    def connect_to_server(self):
        ...
        print('Attempting to connect to server using ip-address: ' +
              self.IP_address+':33000...')
    ...
    def play_tournament(self):
        ...
        while True:
            ...
            if state == 'Series start':
                unique_player_id = ast.literal_eval(
                    self.ssl_sock.recv(1024).decode('utf8'))
                player_id_map = ast.literal_eval(
                    self.ssl_sock.recv(1024).decode('utf8'))
                num_games = ast.literal_eval(
                    self.ssl_sock.recv(1024).decode('utf8'))
                game_params = ast.literal_eval(
                    self.ssl_sock.recv(1024).decode('utf8'))
                series_player_id = [
                    p[1] for p in player_id_map if p[0] == unique_player_id][0]
                self.handle_series_start(
                    unique_player_id, series_player_id, player_id_map, num_games, game_params)
            ...
            # We received a game end message
                ...
                self.handle_game_over(ast.literal_eval(
                    winner), ast.literal_eval(end_state))
            ...
            # We received an illegal action message
                ...
                self.handle_illegal_action(ast.literal_eval(
                    state), ast.literal_eval(illegal_action))
                break
